...
\newcommand{\wf}{\texttt{FE-WorkflowTools}}
...
Below is a list of references related to the AMBER DD Boost package, 
as of \date. 
...
AMBER20, and can be installed by running the \textit{install\_\addb.sh}
script located in the \addb directory.
...
\tk can be installed by running the \textit{INSTALL.sh} script located 
inside \tk directory.
...
\vspace{0.1cm}
\section{Contents of the Alchemical\_fe repository}
\vspace{0.1cm}

The \textit{Alchemical\_fe} repository contains the following folders:
\begin{itemize}
        \item{\textit{Documentation} - containing documentation specific 
		to AMBER Drug Discovery Boost}
        \item{\textit{Tutorials} - containing tutorials for setting up 
		alchemical free energy simulations using AMBER DD Boost}
        \item{\textit{Examples} - containing test cases for RBFE and 
		RSFE calculations}
        \item{\textit{bin} - containing scripts related to workflow tools 
		to help set up alchemical free energy simulations 
		using Amber Drug Discovery Boost}
\end{itemize}
...
\begin{itemize}
	...
	\item{\textit{FE-ToolKit} must be installed when using \wf in "analysis" mode.}

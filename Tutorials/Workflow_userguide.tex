...
\title
{\bf User guide for using workflow tools for automated Production Free Energy Simulation Setup and Analysis (\textit{ProFESSA})using AMBER Drug Discovery Boost}
...
\vspace{1cm}
\section{Purpose of the \textit{Workflow Tools}}
\vspace{1cm}

\textit{Workflow Tools} are a set of scripts that are designed to 
facilitate the setup, execution, and analysis of alchemical 
free energy simulations (AFE) using AMBER DD Boost. Currently, the 
\textit{Workflow Tools} can be used to perform relative binding
...
The \textit{Workflow Tools} can then be used to generate a hierarchy of directories 
containing relevant parameter, coordinate, and AMBER input files and job submission scripts. 
The \textit{Workflow Tools} can also be used 
to analyze the free energy simulations using \textit{FE-ToolKit}


\vspace{1cm}
\section{Initial requirements of using the \textit{Workflow Tools}}
\vspace{1cm}

Following conditions must be met before using the \textit{Workflow Tools} -
...
\vspace{1cm}
\section{Usage of the \textit{Workflow Tools}}
\vspace{1cm}

The \textit{setup\_fe} script represents the main executable of the \textit{Workflow Tools}, 
and can be 

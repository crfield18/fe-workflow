...
while read line; do
        varlist=(system translist nlambda protocol mapping ntrials cutoff mincyc nstlimnvt nstlimnpt repex nstlimti numexchgti hmr scalpha scbeta gti_sc_add gti_scale_beta gti_cut gti_cut_sc_on gti_cut_sc_off gti_lam_sch gti_ele_sc gti_vdw_sc gti_cut_sc gti_ele_exp gti_vdw_exp stage ticalc partition nnodes ngpus wallclock) 
...
EOFN
	...
	if [ "${gti_sc_add}"    -lt 1 ] || [ "${gti_sc_add}"       -gt 5 ]; then echo "Acceptable values for \"gti_sc_add\" are 1, 2(Recommended), 3, 4, and 5" && exit 0; fi
        ...
        if [ "${gti_cut_sc}" != 0 ] && [ "${gti_cut_sc}" != 1 ] && [ "${gti_cut_sc}" != 2 ]; then echo "Acceptable values of \"gti_sc_addcut\" are 0(default), 1, 2" && exit 0; fi
        if [ "${gti_cut_sc}" -eq 1 ] || [ "${gti_cut_sc}" -eq 2 ]; then
                if [ -z "${gti_cut_sc_on}" ] || [ -z "${gti_cut_sc_off}" ]; then echo "if \"gti_sc_addcut\" is set to 1 or 2, gti_cut_sc_on and gti_cut_sc_off must be defined." && exit 0; fi
        fi
        if [ "${cutoff}" -lt "${gti_cut_sc_on}" ] || [ "${cutoff}" -lt "${gti_cut_sc_off}" ] || [ "${gti_cut_sc_off}" -lt "${gti_cut_sc_on}" ]; then echo "Should be \"cutoff\" >= \"gti_sc_addoff\" > \"gti_sc_addon\" " && exit 0; fi
...
EOF
				...
				if [ ! -d ${path}/${system}/${protocol}/${stA}~${stB}/${s}/build ]; then
					...
					cd ${path}/${system}/${protocol}/${stA}~${stB}/${s}/build
						...
						if [ "${ticalc}" == "rbfe" ]; then
							writetemplate_rbfe $nlambda $cutoff $mincyc $nstlimnvt $nstlimnpt $repex $nstlimti $numexchgti $timask1 $timask2 $scmask1 $scmask2 $scalpha $scbeta $gti_sc_add $gti_scale_beta $gti_cut $gti_cut_sc_on $gti_cut_sc_off $gti_lam_sch $gti_ele_sc $gti_vdw_sc $gti_cut_sc $gti_ele_exp $gti_vdw_exp $stA $stB
						else
							writetemplate_rsfe $nlambda $cutoff $mincyc $nstlimnvt $nstlimnpt $repex $nstlimti $numexchgti $timask1 $timask2 $scmask1 $scmask2 $scalpha $scbeta $gti_sc_add $gti_scale_beta $gti_cut $gti_cut_sc_on $gti_cut_sc_off $gti_lam_sch $gti_ele_sc $gti_vdw_sc $gti_cut_sc $gti_ele_exp $gti_vdw_exp $stA $stB
						fi
						##########################################

...
GitLab account to: Shi Zhang at sz550@rutgers.edu or Darrin York 
...
\hline
combine\_aq        & string          & Specifies whether the aq and com should be run as a single job. Only applies to equil\_type=2. Acceptable values - \texit{true}, \textit{false} & combine\_aq = false \\
\hline
...
\hline
max\_dt      			& float         & Specifies the maximum timestep (default is 0.002 for not hmr, 0.004 for hmr)  & max\_dt=0.002/0.004  \\ 
\hline
ntwx      			& integer         & Specifies the frequency for which TI trajectory frames will be saved  & ntwx=0  \\ 
\hline
ntwx\_equil      			& integer         & Specifies the frequency for which TI trajectory frames will be saved for equil runs (note - careful choice recommended)  & ntwx=0  \\ 
\hline
ntwx\_ep 			& integer  		  & Specifies the output frequency for the end point trajectories & ntwx\_ep=ntwx \\
\hline
...
\hline
equil\_type         & integer         & Specifies whether a single equilibration (equil\_type=1) or a trial-dependent equilibration (equil\_type=2) will be performed. Acceptable values - \textit{1}, \textit{2}. & equil\_type=2  \\
\hline
source\_header	   & string 		 & Specifies the path to the header file that will be sourced before running the TI simulations. Acceptable values - \textit{null}, \textit{path to header file}. & source\_header=null  \\
\hline
